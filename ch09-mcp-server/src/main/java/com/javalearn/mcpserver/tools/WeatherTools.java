 ...
 * <p>
 * 使用 @McpTool 注解定义工具（Spring AI 2.x 中 MCP Server 专用注解，
 * 与本地函数调用的 @Tool 不同），MCP Server Boot Starter
 * 会自动扫描并注册为 MCP 工具。
 */
@Service
public class WeatherTools {
    ...
    @McpTool(description = "查询指定城市的当前天气情况，包括温度、湿度和天气状况")
    public String getWeather(@McpToolParam(description = "城市名称，如：北京、上海、深圳") String city) {
    ...
    @McpTool(description = "查询指定城市未来几天的天气预报")
